# -*- coding: utf-8 -*-
"""offtargetmodelsexperiments_4x23.ipynb ."""
...
def plotConfusionMatrix(y_test, ypred):
    confData = {'y': y_test, 'ypred': ypred}
    confdf = pd.DataFrame(confData, columns=['y', 'ypred'])
    confMatrix = pd.crosstab(confdf.y, confdf.ypred,
                             rownames=['Target'],
                             colnames=['Predicted'])
...
def getClassificationMetrics(y_test, yscore, ypred):
    ...
    for n in posLabel:
        print("%-40s" % ("F1 Score Class " + str(n) + " :"), 
              "{:.3f}".format(f1_score(y_test,
                                       ypred,
                                       pos_label=n)))
        print("%-40s" % ("Recall Score Class " + str(n) + " :"), 
              "{:.3f}".format(recall_score(y_test,
                                           ypred,
                                           pos_label=n)))
...
if ismodelsaved is True:
    ffn3 = tf.keras.models.load_model('/saved_model_4x23/ffn3_4x23')
    ffn = tf.keras.models.load_model('/saved_model_4x23/ffn5_4x23')
    ffn10 = tf.keras.models.load_model('/saved_model_4x23/ffn10_4x23')
    model_cnn3 = tf.keras.models.load_model('/saved_model_4x23/cnn3_4x23')
    model_cnn5layers = tf.keras.models.load_model('/saved_model_4x23/cnn5_4x23')
    model_cnn10layers = tf.keras.models.load_model('/saved_model_4x23/cnn10_4x23')
    model_cnnreplica = tf.keras.models.load_model('/saved_model_4x23/cnnlinn_4x23')
...
if imgrows == 4:
    if nexp == 1:
        loaddata = pkl.load(
            open('encoded4x23.pkl', 'rb'), encoding='latin1')
    elif nexp == 2:
        loaddata = pkl.load(
            open('encoded4x23linn.pkl', 'rb'), encoding='latin1')
    elif nexp == 3:
        loaddata = pkl.load(
            open('encoded4x23withoutTsai.pkl', 'rb'), encoding='latin1')
    elif nexp == 4:
        loaddata = pkl.load(
            open('guideseq4x23.pkl', 'rb'), encoding='latin1')
else:
...
plt.legend(loc='best')
plt.savefig('roccurvecrispr4x23.pdf')
...
print('RF:', np.round(accuracy_score(y_test, ypred[7]), 3))
...
print('RF:', np.round(brierScore(y_test, yscore[7]), 3))
...
print('RF:', np.round(f1_score(y_test, ypred[7]), 3))
...
print('RF:', np.round(
    precision_score(y_test, ypred[7], zero_division=0), 3))
...
print('RF:', np.round(recall_score(y_test, ypred[7]), 3))

# display the top results based of class 1 based on ascending proba of class 1

predictions = model_cnn3.predict(x_testcnn)
# predictions = ffn3.predict(x_testffn)
...
# we import the pkl file containing the data
if imgrows == 4:
    loadguideseq = pkl.load(open('guideseq4x23.pkl', 'rb'), encoding='latin1')
else:
    print('refer to experiments 8x23 py file')
...
# plt.title('ROC curve')
plt.legend(loc='best')
plt.savefig("roccurveguideseq4x23.pdf")
...
pre_ffn3, rec_ffn3, _ = precision_recall_curve(
    y_guideseqffn[:, icol],
    ffn3.predict(x_guideseqffn)[:, icol], pos_label=1)
pre_ffn, rec_ffn, _ = precision_recall_curve(
    y_guideseqffn[:, icol],
    ffn.predict(x_guideseqffn)[:, icol], pos_label=1)
pre_ffn10, rec_ffn10, _ = precision_recall_curve(
    y_guideseqffn[:, icol],
    ffn10.predict(x_guideseqffn)[:, icol], pos_label=1)
pre_cnn3, rec_cnn3, _ = precision_recall_curve(
    y_guideseqcnn[:, icol],
...
print('RF:', np.round(brierScore(y_guideseq, yscore[7]), 3))
...
print('RF:', np.round(accuracy_score(y_guideseq, ypred[7]), 3))
...
print('RF:', np.round(f1_score(y_guideseq, ypred[7]), 3))
...
print('RF:', np.round(
    precision_score(y_guideseq, ypred[7], zero_division=0), 3))
...
print('RF:', np.round(recall_score(y_guideseq, ypred[7]), 3))
...
# display the top results based of class 1 based on ascending proba of class 1
# best model is FNN5
predictions = ffn.predict(x_guideseqffn)
classpredictions = np.argmax(predictions, axis=1)
